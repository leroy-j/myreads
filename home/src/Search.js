  ...
  */
  getBtn(haystack,needle){
    ...
    if(res.length)
      ...
      return (<button className="btn default" name={needle.id}	onClick={this.props.onAddBook}>Add to my reads</button>)
 }
     	...
     	<div className="search-books-bar">
        	<Link className="close-search" to={{ pathname:'/'}}>Close</Link>
         	<div className="search-books-input-wrapper">
            	<input type="text" onChange={this.props.doSearch} placeholder="Search by title or author"/>
					</div>
      </div>
	<br /><br />    	
	   ...
	   <div className="modal-body">
	     <div className="modal-main">
            ...
            <div className="modal-content">
							<p className="preview-book-title">{ selectedSearch.title }</p>	
		      	<div className="left">
							<div className="book-cover" href="/preview?id={book.id}" style={{width: '128px', height: '192px', backgroundImage: 'url("'+selectedSearch.imageLinks.thumbnail+'")'}}></div>
								<p>
									{
  									((selectedSearch.industryIdentifiers).length && 
    								(selectedSearch.industryIdentifiers.map( 
		  								(mod)=><span key={mod}>{(mod.type).toString().toUpperCase()}:	{mod.identifier} </span>
	  								)
  									))	
 									}
								</p>
